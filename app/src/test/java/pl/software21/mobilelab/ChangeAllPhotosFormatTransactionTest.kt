...
class ChangeAllPhotosFormatTransactionTest {
    ...
    fun execute() {
        ...
        ChangeAllPhotosFormatTransaction(changedFormat, cart).execute()

        assertEquals(changedFormat, cart.getPhoto("id")?.format)
        assertEquals(changedFormat, cart.getPhoto("id2")?.format)
    }
